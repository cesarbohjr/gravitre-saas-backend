...
| Phase 5 ML | **UNHELD** — v1 shipped (CRM outcomes + ranked heuristics). **Churn tip PASS**; **CF v1 soft-rank tip PASS** 2026-07-19 — `phase0-cf-churn-ml.md` |
...
- CF v1: item-affinity soft-rank after heuristics / before dismiss; ≥50 interactions/30d gate. **PASS** — heuristics HTTP 200 `cfRanked:true` gate ready @ tip sha `9d5451a5…` (`phase5-cf-soft-rank-live.json`). Full MF still OOS.
