...
| Multi-jurisdiction queries route to zero knowledge packs | **OPEN, real retrieval gap, unfixed.** `classify_knowledge_query` on the Ontario-vs-California breach-deadline query resolves `juris=['US-CA','CA-ON','US-federal']` and `depts=[]`, and packs are selected by department, so it retrieves **no legal evidence** for the most obviously legal question in the test set. The CRAG loop handled it correctly — INCORRECT, 2 rounds, honest shortfall — which is the trap: **an iterative loop cannot repair a routing miss, only spend rounds discovering nothing is there.** A jurisdiction fallback is a retrieval-policy change Phases 1–5 did not authorise | unassigned |
| Org RAG keyword migration `20260903100000` not applied | **OPEN by design.** Production schema change, so it needs an explicit human decision rather than an agent reading it out of "complete Phase 5". Code degrades to the old unfiltered fetch with a WARNING, so the deployed tip is safe. Measured impact of applying today is nil (1 chunk platform-wide, 0 scopes over cap); the value is entirely in the first real corpus | Cesar |
