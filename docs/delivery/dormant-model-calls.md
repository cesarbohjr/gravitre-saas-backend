...
| Org RAG keyword migration `20260903100000` not applied | **CLOSED 2026-09-03 — authorised, then found already applied.** The register entry was stale by the time it was written. Verified in prod rather than trusted from the migration history: `content_tsv` present as `to_tsvector('english', COALESCE(content,''))` with `attgenerated='s'` matching the Fabric exactly, `context_prefix` present, both indexes present, recorded **twice** (`20260903100000` and `20260903180147`, idempotent so harmless). **Exercised, not just inspected:** `fetch_bm25_corpus` against prod returns `reach=fts_no_match` for real terms — the pre-select ran and matched nothing, with no swallowed `TypeError` and no silent fallback, which is the exact failure mode that kept the Fabric arm dead for months. Still latent: 1 chunk platform-wide, 0 scopes over cap | Cesar |
