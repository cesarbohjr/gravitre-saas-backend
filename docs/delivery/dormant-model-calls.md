...
| Keyword arm has no `ts_rank` relevance ordering | **CLOSED 2026-09-04.** Postgres RPC `search_knowledge_chunks_fts` orders by `ts_rank`; retrieval prefers RPC, records `retrieval_health.fts_ordered`, scales rank into semantic_score for fusion. postgrest fallback retained until migration applied everywhere | Cesar |
| Prompt 1 (CRAG) and Prompt 2 (Context Engine) | **DEFERRAL SUPERSEDED 2026-09-03 by direct instruction.** Prompt 1 **BUILT** (`fe53c888`, `985149ad`) — see `crag-iterative-loop.md`. Prompt 2 is **LIVE ACTIVE**: shared cross-source normalization/ranking shipped at `db073571`; production shadow then active probes both returned HTTP 200 with no stream errors, and the active audit reported `mode=active`, 3 selected sources, 1,058/12,000 tokens, and duplicate kernel Fabric suppression. `CROSS_SOURCE_CONTEXT_ENGINE_ENABLED=true` in Railway production. | Cesar |
| Retrieval Layer 1 (contextual chunk enrichment) ABSENT | **CLOSED DEFERRED 2026-09-04.** Built at `985149ad`, flag OFF by design. Synthetic before/after only; real corpus proof blocked at 1 chunk platform-wide. Re-open when a real org corpus exists | Cesar |
| `memory_temporal_supersede_failed` logs empty cause | **CLOSED 2026-09-04.** PostgREST errors often stringify to `""`; `_format_exc()` now logs type + repr and supersede failures include org/key/old/new ids | Cesar |
...
| CRAG loop actions not yet proven in production | **CLOSED 2026-09-03 — LIVE PASS (probe-derived), 10/10.** Organic discard: **NOT PROVEN** (0 non-test actors); probe-derived discard **PASS** — 67 audit rows, latest `61d2178e…` @ `2026-09-04T06:33:11Z`. See `crag-discard-organic-evidence.json` | Cesar |
| Phase 5 probe passed its own discard check over an empty evidence set | **CLOSED 2026-09-03, Class B in the verification harness itself.** First run was 6/8 and the two failures were the instrument, not the code: turns ran with no packs assigned, so every turn retrieved 0 rows. The `forced_discard_executed` check **passed** on a counter increment while `discarded_rows` was **0** — the branch ran and destroyed nothing. Checks now assert outcomes (`discarded_rows > 0`, `refined_to < refined_from`), not that a line was reached. The simple-turn check separately failed a turn that was genuinely free, for skipping by the wrong door | Cesar |
| Multi-jurisdiction queries route to zero knowledge packs | **SUPERSEDED — the diagnosis was one layer too low, Class A in this program's own report.** "Jurisdiction-only route yields nothing" described the symptom and never asked why no department was detected. Real cause below | — |
| Fabric router matched department keywords by naked substring | **FIXED 2026-09-03, mutations 10/10.** Two live directions. **Missed:** `"statutory"` is not a substring of `"statute"`, `"regulator"` not of `"regulation"`, and **privacy vocabulary was absent from the legal list entirely** — `privacy`, `HIPAA`, `GDPR`, `CCPA`, `breach notification` all routed nowhere, so the most ordinary privacy question retrieved no legal evidence. **Fired wrongly:** no word boundary, so `law` matched inside **flawed**/**outlaw**, `incident` inside **coincident**, `sec` inside **secondary**/**security**/**cybersecurity**, routing security questions to finance. **The obvious fix was measurably worse than the defect:** plain word boundaries remove 17 accidents and destroy 47 wanted inflections (`prospects` 21, `prospecting` 9, `msps` 10, `cybersecurity` 7) — visible only because reachability was measured on **1982 real user messages** first. (Committed at `5a6abb4e` as "38", which dropped `prospecting` from the sum; corrected here. The direction of the claim is unchanged and slightly stronger.) Rule is asymmetric: left boundary always, suffix allowed except for acronyms. Before/after on the same corpus: **+24 newly routed (all legal), −17 finance accidents, zero losses elsewhere.** `compliance` deliberately excluded — it would pull SOC 2 / NIST traffic into legal | Cesar |
| Exact-match exception list contained keywords that do not exist | **CLOSED 2026-09-03, caught by its own guard on first run.** `phi` and `dpa` were added to `_EXACT_ONLY_KEYWORDS` on the assumption they were vocabulary. They are not, so the entries protected nothing while reading as though they did. `test_every_exact_only_keyword_is_actually_in_the_vocabulary` fails on any such entry | Cesar |
| Org RAG keyword migration `20260903100000` not applied | **CLOSED 2026-09-03 — authorised, then found already applied.** Verified in prod: `content_tsv`, `context_prefix`, and both indexes are present; `fetch_bm25_corpus` returns `reach=fts_no_match` for real terms with no silent fallback. Migration history was subsequently canonicalized to `20260903100000`; the generated duplicate stamp was removed without rerunning DDL. Still latent: 1 chunk platform-wide, 0 scopes over cap. | Cesar |
...
| Three Railway configs declare three different healthcheck timeouts | **OPEN, latent, not asserted either way.** `railway.json:10` = **30s** (builds `backend/Dockerfile`, watches `backend/**`), `backend/railway.toml:40` = **600s**, `backend/backend/railway.toml:12` = **300s**. Which one Railway reads depends on the service root, which is not visible from the repo. Prod deploys **are** verifiably green — the deploy workflow polls live `/health` for a matching `git_sha` — so the effective value is evidently tolerant. But 30s is below measured startup, so if service root ever resolves to the repo root, every deploy fails healthcheck and burns 3 restarts. Reconcile to one file | Cesar |
