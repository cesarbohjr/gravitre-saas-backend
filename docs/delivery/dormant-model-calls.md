...
| CRAG loop actions not yet proven in production | **CLOSED 2026-09-03 — LIVE PASS (probe-derived), 10/10.** `prove-crag-phase5-live.py` against prod `985149ad`. Strongest result is unforced: a hard single-jurisdiction turn with the **real** assessor classified `INCORRECT`, spent 2 rounds and **destroyed 6 real evidence rows**; `finalStanceInferred=false`; dedicated row and nested block agree. Forced branches destroyed 6 rows and narrowed 6→1. Simple turn 901ms / 0 assessor calls. **Probe-derived, not organic** — no real user has triggered a discard | Cesar |
