...
| 3 | F6 HubSpot follow-up | **CLOSED** | Cause ≠ wrong-endpoint: HubSpot/Apollo membership eventual consistency (size 0 then 1). Fix: shared retry/backoff + HubSpot settle read in `collection_population_verify._follow_up_membership_count`. Live both `follow_up_membership_confirmed` — Apollo list `6a73b32afaa7f90020558cfe`, HubSpot list `41` (`docs/delivery/f6-collection-population-verify-live.json`, tip `8bde6113` at run). Standing CI: `test_f6_hubspot_follow_up_membership.py` (+ Apollo retry unit). |
| 4 | A5d progressive + write governance | **CLOSED** | Gap: write intents used `handoff_short_circuit` (never LIVE). Fix: writes run `execute_task_streaming`. Live tip `f586c1fe`: `progressive_disclosure=true` (4001 B stubs), Apollo write `outcome_kind=connector_tool_proposal` + “Reply **yes**”, path `execute_task_streaming` — conv `40a3dc42…` / `f795d1cc…` (`a5d-extension-progressive-live.json`). |
| CI | Backend pytest | **GREEN on tip `f586c1fe`** | GitHub Actions Backend (pytest) **success** on `f586c1fe`. Local full suite **4386 passed, 3 skipped**. Web lint + pip-audit were red on that tip (pre-existing hooks/`cryptography`); follow-up commit addresses those. |

**Deploy tip (this closure):** re-stamp after follow-up commit; confirm `/health` `git_sha` matches.
