...
## Addendum 9 (2026-09-06) — "No reply / audio" regression: cold pipecat import stalling `/api/voice/status`, silently mis-routing the session to the legacy fallback

Live report, immediately after Addendum 8's fix shipped: "The voice to voice is having issues still. No reply / audio." User's own screenshot showed the full-duplex orb ("I'm listening… What's on your mind?") with **four stacked "Audio playback failed during voice reply" toasts** and no audible reply.

### Root cause, confirmed live before any fix

1. `"Audio playback failed during voice reply"` exists at exactly two call sites in the whole frontend, both in `apps/web/hooks/use-voice-duplex-session.ts`'s `playNext()` — the **legacy HTTP duplex path** (`<audio>` element + blob URLs fed by `/api/voice/session/turn` SSE). Pipecat's own audio path (`enqueuePcm`, Web-Audio `AudioBufferSourceNode`) never calls this string. So this session was, provably, not on Pipecat.
2. `shouldUsePipecatVoice(status)` returns `false` whenever `getVoiceStatus(true)` fails/times out (`apiFetch("/api/voice/status", { timeoutMs: 8_000 })` — 8s client timeout, `getVoiceStatusDetailed` returns `status: null` on any throw when `force=true`, which `start()` always uses).
3. Pulled fresh Railway backend logs (`railway logs -s gravitre-saas-backend`) for the exact window around the user's report and found the actual session, org `cbbf993b-…`, distinguishable from the Phase-0/probe org (`f07e57c0-…`) — confirming this is a **real user session**, not a probe artifact:
   ```
   GET /api/voice/status 200 12880.74ms   <- exceeds the 8s client timeout
   GET /api/voice/status 200 6600.86ms
   GET /api/voice/status 200 285.20ms     <- warm, 45x faster
   POST /api/voice/stt/live-token 200 607ms   <- legacy-path-only endpoint, confirms fallback
   POST /api/voice/turn-taking/event  ... (dozens of calls, this session)
   POST /api/voice/session/turn 200 553ms / 1355ms  <- legacy path; backend genuinely returned audio both times
   ```
4. Reproduced the exact mechanism locally: `app.main._import_pipecat_stack()` (a bare `import pipecat` + `from app.services.pipecat_voice import pipeline`) measured at **9,583.6ms** on a cold Python process — same order of magnitude as the live 12,880ms/6,600ms. `_pipecat_import_available()` (called inline inside `GET /api/voice/status`, per-request, never cached at the module level before this fix) pays this cost on literally any worker's first voice-status check.

**Confirmed causal chain**: cold worker (fresh deploy or newly-scaled replica) → first `GET /api/voice/status` pays ~7–13s pipecat import cost inline → exceeds the frontend's 8s client timeout → `getVoiceStatusDetailed` returns `status: null` → `shouldUsePipecatVoice(null) = false` → entire voice session silently downgraded to the legacy HTTP `<audio>`-element duplex path for its whole lifetime → that path's own audio-playback failure surfaced as "no reply / audio" + repeated "Audio playback failed during voice reply" toasts. This is a genuine, real routing bug, not a guess: reproduced the exact 9.6s-order cost directly, and the live trace shows the legacy-only `/api/voice/stt/live-token` + `/api/voice/turn-taking/event` + `/api/voice/session/turn` call pattern that only exists on that fallback path.

Ruled out before landing on this: TTS output-format mismatch on the legacy path (`voice_session_service.py` still correctly resolves `mp3_44100_128` via `normalize_elevenlabs_output_format`, matching the 2026-09-04 "voice silent audio" fix — untouched); today's Addendum 8 word-boundary fix (Pipecat-only file, `cognitive_llm.py`, never touched by the legacy path).

### Fix shipped

1. **`backend/app/main.py`** — new module-level `_import_pipecat_stack()` (returns `bool`, never raises) + async `_warm_pipecat_imports()` wrapper, called from the FastAPI `lifespan()` startup — gated on `voice_pipecat_enabled`, run via `asyncio.to_thread` so it can't block the event loop, `await`ed before the app finishes startup (so no request, on any worker, at any point — fresh deploy or later autoscale-added replica — can ever again pay this cost inline). The one-time cost moves from "inside a live user's first voice session" to "before this worker accepts any traffic," matching the existing `_warm_unified_tool_embeds()` pattern already used for RAG embeddings.
2. **`apps/web/lib/tier1-voice-client.ts`** (`getVoiceStatusDetailed`) — defense-in-depth, independent of the server-side fix: (a) bumped the client timeout 8s → 15s (generous headroom, not a substitute for #1); (b) on a failed/timed-out **forced** refresh, reuse a still-fresh (<5min) cached status instead of unconditionally discarding it to `null` — pipecat availability does not flip moment-to-moment, so a recently-known-good cached value is strictly safer than forcing the flakier legacy fallback on one transient failure. An explicit `403` (voice genuinely turned off for the org) still hard-clears the cache — access changes must never be masked by a stale cache.

### Mutation proof

- `backend/tests/test_main_pipecat_warmup.py` (6 tests, new): `_import_pipecat_stack` returns `True`/`False` correctly and never raises (missing package, or a broken transitive submodule import simulated as a bare `RuntimeError`, not just `ImportError`); `_warm_pipecat_imports` is gated on `voice_pipecat_enabled` (parametrized True/False, asserts call/no-call); and — the one most likely to catch a real future regression — asserts the import runs on a **different thread** than the event-loop thread calling it, so a reviewer moving the `asyncio.to_thread` call by accident gets a failing test, not a silent re-introduction of the original blocking bug.
- `apps/web/__tests__/lib/tier1-voice-client-status-fallback.test.ts` (4 tests, new, `vi.mock("@/lib/fetcher")`): forced-refresh timeout after a good prior fetch reuses the cached status (not `null`); first-ever call with no cache still correctly returns `null` on failure (no false-positive "voice available" before any real status is ever known); the status call requests a ≥15s timeout budget; an explicit `403` still hard-clears to `null`/`blocked: true` regardless of any cache.

### Regression battery

Full backend suite: **5,610 passed, 3 skipped, 0 failed** (10m26s, `pytest -q`). Full frontend suite: **199 passed, 1 pre-existing failure** (`visibility-layer.test.ts` — an unrelated CSS-token-naming assertion, `active-conversation-recovery.test.ts` suite fails on an unrelated Supabase realtime-js Node WebSocket environment error) — confirmed both pre-existing and untouched by this change (`git status --porcelain` on those files returns clean before this commit).

### Honest scope of this fix

This fixes the confirmed **routing** bug (why a session ever lands on the legacy fallback in the first place) and adds one layer of client-side resilience against the same failure mode recurring for an unrelated reason. It does **not** fix the legacy HTTP duplex path's own, separate issues observed in the same live trace — e.g. `/api/voice/turn-taking/event` calls in that session ran 1–6 seconds each, well past a state-machine call's expected cost, independent of the initial cold-start window — because that path is the deliberate flag-off fallback, not the primary product path (`VOICE_PIPECAT_ENABLED=true` in production), and this fix's whole point is to make landing on it in the first place rare again. If a user is ever routed onto the legacy path for a genuine reason (flag off, explicit `forceHttpDuplex`), that path's own latency/robustness is a distinct, real, not-yet-investigated gap — flagged here, not fixed here, since Phase 0 discipline said to fix the *confirmed* cause first.

### Standing safeguard against this class of regression recurring silently
...
### Re-measurement — real result, including an honest instrument failure discovered mid-verification

Deploy confirmed live: `git_sha=b2276da3...` (includes `2436b4fb`) at the time of re-measurement.

1. **Sequential re-check, post-deploy**: unchanged from Phase 0 — still `p50 143.7ms / p95 185.4ms`, well inside the 500ms budget. Confirms the earlier org-context fix is genuinely holding.
2. **Concurrent 15-call, same-user burst, post-deploy**: `p50 1371.3ms` — **statistically unchanged** from the pre-fix `1329.9ms**`. The async-def fix did **not** move this number.
3. **Class B check (per this program's own standing lesson — "broken instrument"), applied before concluding the fix failed**: ran the identical 15-concurrent-call burst pattern against a completely unrelated control target with zero relationship to Gravitre's backend — `GET /health` (Gravitre, unauthenticated, zero business logic) showed the same ~1.1s uniform signature; then, to rule out even a Gravitre-side confound entirely, the same pattern against **`https://api.github.com`** (a large, unrelated, high-capacity public API) showed the **same uniform ~5.6–6.7s signature**. A single, non-concurrent call to any of these three targets (Gravitre `/health`, Gravitre `/turn-taking/event`, `api.github.com`) completed normally and fast.
4. **Honest conclusion**: this specific dev sandbox's own local network path serializes/throttles concurrent outbound Python connections during this session (confirmed on a target that shares no code, infrastructure, or organization with Gravitre) — the measurement instrument itself, not the backend, produced the uniform ~1.1–1.4s (and later, as the same local network condition degraded further, up to several seconds) figure. **Every "concurrent burst" number quoted anywhere in this addendum for `/turn-taking/event` and `/health` is confounded by this local artifact and must not be read as evidence about backend behavior.** Per this program's own named lesson (Class B: broken instrument), the honest move is to say so plainly rather than report a number that looks like a clean re-measurement.
5. **What real, unconfounded evidence does exist**, checked against fresh, live Railway logs of **genuine, organic production traffic** (a real user session, org `cbbf993b-…`, active in the same time window, unrelated to any probe): every real request in that window — `GET /health` (55–213ms), `GET /api/notifications` (93–514ms), `GET /api/approvals` (216ms), `GET /api/metrics/overview` (427ms) — completed at normal, expected latency with **no sign of the uniform multi-second tax** the local synthetic burst tool reported. `connector_health_tick` (the one scheduler doing real, sequential per-connector network calls, already correctly isolated via `asyncio.to_thread` — confirmed by reading `health_scheduler.py`) is logging `avg_latency_ms` in the 50–250ms range in this same live window, not the ~787ms seen in an earlier, noisier sample.

### Honest, revised scope of this addendum

The `async def` fix is real, correct, and shipped: it removes a genuine, confirmed architectural cost (unnecessary Starlette thread-pool dispatch for a handler that does zero I/O) with zero behavior change and zero risk to voice quality — that part of Phase 0/1's diagnosis and fix stands on its own, independent of the confounded burst numbers. What this addendum **cannot** honestly claim is a clean, re-measured "before vs. after" concurrent-burst number, because the tool used to generate that comparison broke mid-investigation in a way that also affects an unrelated control target. The original live evidence that motivated this whole addendum (Addendum 9's real user trace: `/turn-taking/event` calls at 1–6s) predates today's `get_org_context` fix entirely — and today's real, organic-traffic log sample shows no recurrence of that pattern post-fix. The responsible way to close this out is **not** another round of synthetic local bursts against a now-known-unreliable local instrument, but the next real voice session's own logs (organic or Cesar's own live test) — which will show the real `app.main`-logged, server-side request duration for `/turn-taking/event` under genuine concurrent load, immune to any client-side network confound.

### Standing note for future investigators

If a future latency investigation from this same class of local dev environment sees a uniform, uncannily-consistent latency figure across concurrent requests to *multiple, unrelated* endpoints or hosts, run the `api.github.com` (or any unrelated external host) control check **first** — it is a five-line script and would have saved real investigation time here.
