...
**Tip at build:** will be updated after deploy.  
...
| Phase 0 live TTS 200 + real audio bytes | **PENDING FUNDED-ACCOUNT VERIFICATION** (same `ELEVENLABS_API_KEY`; prior tip returned 502 wrapping upstream 402) |
| Phase 0 latency baselines on prod | **PENDING FUNDED-ACCOUNT VERIFICATION** |
| Phase 0 text TTFT gate on live tip | **PENDING** re-confirm after deploy |
...
| Phase 3 naming / spoken register / library / design APIs | **BUILT**; live preview/audio **PENDING FUNDED-ACCOUNT VERIFICATION** |
...
| Phase 7 COGS math + metering code + Billing UI card | **BUILT**; Stripe meter seed needs `STRIPE_SECRET_KEY` run; live overage **PENDING** after env attach |
