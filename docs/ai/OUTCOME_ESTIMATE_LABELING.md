...
|------|----------|----------|--------|
| **Estimate** | Catalog hours saved, adopted hours, ROI realization, Meson/optimization impact | `Estimate` badge or "Estimated …" in copy | Publisher metadata, catalog math, heuristics |
| **Operational** | Workforce tasks completed/failed, lite results counts, workflow success rate | `Operational` badge or "(operational)" in label | `agent_jobs`, `workflow_runs`, audit/tool events |
| **Deferred** | Time-on-task, before/after pilot ROI | Not shown as live | STA-289 roadmap |
