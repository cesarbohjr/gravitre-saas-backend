...
try {
    ...
    Write-Host "Clio OAuth status ($uri):"
    $r | ConvertTo-Json -Depth 5
    if ($r.configured -and $r.encryptionConfigured) {
        Write-Host "READY - Connect Clio at https://gravitre.app/connectors" -ForegroundColor Green
        exit 0
    }
    Write-Host "NOT READY - set CLIO_CLIENT_ID/SECRET on Railway" -ForegroundColor Yellow
    exit 1
} catch {
