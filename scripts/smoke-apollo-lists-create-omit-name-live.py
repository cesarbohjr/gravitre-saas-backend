#!/usr/bin/env python3
"""Live smoke: apollo.lists.create with list_name only (modality defaulted).

Reproduces wave67 omit-name approve failure shape. Writes
docs/delivery/apollo-lists-create-omit-name-live.json
"""
...
OUT = REPO / "docs" / "delivery" / "apollo-lists-create-omit-name-live.json"
...
def main() -> int:
    ...
    list_name = f"gravitre-omit-name-smoke-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
    ...
    result = invoke_tool(ctx, "apollo.lists.create", {"list_name": list_name})
    ...
    report = {
        "scenario": "omit-name: list_name only, modality defaulted to contacts",
        "started_at": started,
