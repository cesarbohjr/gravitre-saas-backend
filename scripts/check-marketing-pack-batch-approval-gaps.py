#!/usr/bin/env python3
"""STA-290: Marketing Operations Pack batch + partial approval audit (Phase 5.2).

Usage:
  npm run marketing-pack:batch-approval-gaps
  python scripts/check-marketing-pack-batch-approval-gaps.py --json docs/delivery/marketing-pack-batch-approval-gaps-latest.json
"""
...
from app.marketplace.marketing_pack_batch_approval_gaps import audit_marketing_pack_batch_approval  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(description="STA-290 marketing pack batch approval audit")
    ...
    print(
        f"STA-290 batch approval: {summary['partial']} partial, "
        f"{summary['missing']} missing, {summary['exists']} exists"
    )
    print(f"Recommendation: {report['recommendation']}")
    print(f"Runtime shipped: {'yes' if summary['runtimeShipped'] else 'no'}")
