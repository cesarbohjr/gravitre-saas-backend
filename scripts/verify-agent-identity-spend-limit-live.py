"""Live proof: Agent Identity spend limit blocks over-limit writes."""
...
OUT = REPO / "docs" / "delivery" / "agent-identity-spend-limit-live.json"
BASE = os.environ.get(
    "BACKEND_URL",
...
def _resolve_actor_and_agent(env: dict[str, str], org_id: str) -> tuple[str, str]:
    ...
    agent = (env.get("OAUTH_SMOKE_AGENT_ID") or env.get("SMOKE_AGENT_ID") or "").strip()
    if actor and agent:
        return actor, agent
    try:
        ...
        if not actor:
            ...
            actor = str((admin or (rows[0] if rows else {})).get("user_id") or "")
        if not agent:
            agents = (
                client.table("agents")
                .select("id")
...
def main() -> int:
    ...
    if secret and actor_id and agent_id:
        try:
            resp = httpx.post(
                f"{BASE}/api/internal/ops/agent-identity-spend-smoke",
                headers={"X-Internal-Secret": secret},
                json={"org_id": org_id, "actor_id": actor_id, "agent_id": agent_id},
                timeout=120.0,
            )
