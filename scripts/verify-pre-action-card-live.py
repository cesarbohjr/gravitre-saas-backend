"""Live proof: One Brain #26 pre-action card fields on capability-resolved write."""
...
OUT = REPO / "docs" / "delivery" / "pre-action-card-live.json"
BASE = os.environ.get(
    "BACKEND_URL",
...
def main() -> int:
    ...
    if secret and actor_id:
        try:
            resp = httpx.post(
                f"{BASE}/api/internal/ops/pre-action-card-smoke",
