...
describe("heuristicRouteIntent", () => {
  it("routes agent creation to execute", () => {
    const result = heuristicRouteIntent("create a new agent for me")
    expect(result.mode).toBe("execute")
  })

  it("routes build workflow requests to execute", () => {
    expect(heuristicRouteIntent("build a workflow that syncs HubSpot deals").mode).toBe("execute")
  })
...
describe("reconcileModelRouteIntent", () => {
  it("overrides mistaken find classification for create requests", () => {
