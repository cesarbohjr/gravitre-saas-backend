...
> = {
  ...
  "customer-success": { icon: Headphones, ring: "text-success", soft: "bg-success/10" },
  hr: { icon: Briefcase, ring: "text-teal-600 dark:text-teal-400", soft: "bg-teal-500/10" },
  operations: { icon: Briefcase, ring: "text-primary", soft: "bg-primary/10" },
}

function themeFor(department?: string | null) {
  const key = (department ?? "general").toLowerCase()
  return (
    DEPARTMENT_THEME[key] ?? {
      icon: Briefcase,
      ring: "text-primary",
      soft: "bg-primary/10",
    }
...
function FeaturedPackCard({ asset, index }: { asset: MarketplaceAssetSummary; index: number }) {
  ...
  const theme = themeFor(asset.department)
