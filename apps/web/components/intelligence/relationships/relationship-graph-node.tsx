"use client"

import { memo } from "react"
import { Handle, Position, type Node, type NodeProps } from "@xyflow/react"
import { cn } from "@/lib/utils"
import { clusterVisualSpec, entityVisualSpec } from "@/lib/relationships-graph/entity-visual"
import type { GraphNodeData } from "@/lib/relationships-graph/types"
import { truncateEntityId } from "@/lib/relationships-graph/utils"

function RelationshipGraphNodeComponent({ data, selected }: NodeProps<Node<GraphNodeData>>) {
  const visual = data.isCluster
    ? clusterVisualSpec(data.entityType)
    : entityVisualSpec(data.entityType, data.isSeeded)
  const Icon = visual.icon

  return (
    <>
      <Handle type="target" position={Position.Left} className="!h-2 !w-2 !border-divide !bg-background" />
      <div
        className={cn(
          "w-[196px] rounded-[var(--np-radius-md)] border px-3 py-2 shadow-[var(--np-shadow)] transition-shadow",
          visual.surfaceClass,
          visual.borderClass,
          selected && "ring-2 ring-[color:var(--g-brand)]/55",
        )}
      >
        <div className="flex items-start gap-2">
          <span
            className={cn(
              "mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-transparent",
              data.isSeeded ? "bg-[color:var(--g-brand-soft)]" : "bg-[color:var(--g-surface-1)]/80",
            )}
          >
            <Icon className={cn("h-4 w-4", visual.iconClass)} aria-hidden />
          </span>
          <div className="min-w-0 flex-1">
            <p className="truncate text-[10px] font-medium uppercase tracking-wide text-[color:var(--g-text-muted)]">
              {data.isCluster ? "Group" : data.entityTypeLabel}
            </p>
            <p className="truncate text-sm font-medium text-[color:var(--g-text-primary)]">{data.label}</p>
            {data.isSeeded ? (
              <p className="mt-0.5 text-[10px] text-[color:var(--g-brand)]">Confirmed knowledge</p>
            ) : data.isCluster ? (
              <p className="mt-0.5 text-[10px] text-[color:var(--g-text-muted)]">Click to expand</p>
            ) : data.secondaryId ? (
              <p className="mt-0.5 truncate font-mono text-[9px] text-[color:var(--g-text-muted)]">
                {truncateEntityId(data.secondaryId, 14)}
              </p>
            ) : null}
          </div>
        </div>
      </div>
      <Handle type="source" position={Position.Right} className="!h-2 !w-2 !border-divide !bg-background" />
    </>
  )
}

export const RelationshipGraphNode = memo(RelationshipGraphNodeComponent)

export const relationshipNodeTypes = {
  relationshipEntity: RelationshipGraphNode,
}
