...
export function GibeDataFlowSection({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section className={`relative border-t border-zinc-200 bg-zinc-50 ${compact ? "py-16 lg:py-20" : "py-24 lg:py-28"}`}>
      <div className="mx-auto max-w-7xl px-6">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-zinc-900 text-balance">{copy.title}</h2>
...
export function TransparencyMetricsSection() {
  ...
  return (
    <section className="relative py-24 lg:py-28 border-t border-zinc-200 bg-white">
      <div className="mx-auto max-w-7xl px-6">
        <motion.div
          initial={{ opacity: 0, y: 24 }}
          whileInView={{ opacity: 1, y: 0 }}
          ...
          className="mx-auto max-w-3xl text-center mb-12"
        >
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-zinc-50 px-4 py-2">
            <BarChart3 className="h-4 w-4 text-zinc-600" />
            <span className="text-sm font-medium text-zinc-700">{copy.badge}</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-zinc-900">{copy.title}</h2>
          <p className="mt-4 text-lg text-zinc-600">{copy.subtitle}</p>
        </motion.div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {copy.tiers.map((tier, i) => (
            ...
            >
              ...
              <p className="mt-2 text-sm leading-relaxed text-zinc-700">{tier.description}</p>
              <ul className="mt-4 space-y-2">
                {tier.examples.map((example) => (
                  <li key={example} className="flex gap-2 text-sm text-zinc-600">
...
export function MarketplaceHighlightsSection() {
  ...
  return (
    <section className="relative py-24 lg:py-28 border-t border-zinc-200 bg-zinc-50">
      <div className="mx-auto max-w-7xl px-6">
          ...
          >
            {copy.packs.map((pack, i) => (
              ...
              >
                ...
                <h3 className="mt-2 font-semibold text-zinc-900">{pack.name}</h3>
                <p className="mt-2 text-sm leading-relaxed text-zinc-600">{pack.description}</p>
              </div>
            ))}
          </motion.div>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {copy.cases.map((useCase, i) => (
            <motion.div
              ...
              className="rounded-2xl border border-zinc-200 p-6 hover:border-zinc-300 transition-colors"
            >
              <div className="text-xs font-semibold uppercase tracking-wider text-emerald-600">
                {useCase.department}
              </div>
              <h3 className="mt-2 text-xl font-semibold text-zinc-900">{useCase.title}</h3>
              <p className="mt-3 text-sm leading-relaxed text-zinc-600">{useCase.description}</p>
    ...
    <section className="relative py-24 lg:py-28 border-t border-zinc-200 bg-zinc-50">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-500 mb-4">Governance</h3>
