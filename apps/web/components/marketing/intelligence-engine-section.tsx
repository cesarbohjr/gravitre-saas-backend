...
export function IntelligenceEngineSection({ variant = "default" }: { variant?: "default" | "compact" }) {
  ...
  return (
    <section
      className={`relative border-t border-border bg-card overflow-hidden ${isCompact ? "py-16 lg:py-20" : "py-24 lg:py-28"}`}
    >
      {!isCompact ? (
        <>
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        >
          ...
          <h2
            className={`font-bold tracking-tight text-foreground text-balance ${isCompact ? "text-3xl sm:text-4xl" : "text-4xl sm:text-5xl"}`}
          >
            {copy.title}
          </h2>
          <p className={`mt-3 text-muted-foreground text-pretty ${isCompact ? "text-base" : "text-lg"}`}>{copy.subtitle}</p>
          <p className="mt-2 text-sm font-medium text-primary">{copy.tagline}</p>
        </motion.div>
          ...
          {copy.layers.map((layer, i) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-2xl border border-border bg-muted/50/80 p-5"
              >
                <div className="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-primary/15">
        ...
        {isCompact ? (
          <motion.p
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mx-auto max-w-3xl rounded-xl border border-border bg-muted/50/80 px-5 py-4 text-sm leading-relaxed text-foreground"
          >
            <span className="font-semibold text-primary">{copy.example.label}: </span>
            {copy.example.text}
          </motion.p>
        ) : (
          <motion.div
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-primary">{copy.example.label}</p>
                <p className="mt-2 text-base text-foreground leading-relaxed font-medium">{copy.example.text}</p>
              </div>
            </div>
          </motion.div>
        )}
