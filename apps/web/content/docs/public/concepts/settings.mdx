...
**Settings → Models** sets **defaults** (workspace, Operator, new agents, fallback). The **Models** product area (`/models`, Command plan) is the production registry with deploy/predict and GIBE built-in catalog at `/models/built-in` — see [Models overview](/docs/concepts/ai-models).
