...
async def test_supersede_stale_orch_for_unrelated_slack(orchestration_service):
    ...
    # Reload must still see awaiting state; post-clear reads return empty.
...
def test_completed_orch_is_not_active_intent(orchestration_service):
    """Terminal connector_orchestration must not keep the orch pipeline alive."""
    task_state = {
        "pending_task": {
            "type": "connector_orchestration",
            "status": "completed",
        },
        "current_plan": {"goal": "Search HubSpot contacts then Slack"},
    }
    assert (
        orchestration_service.is_orchestration_intent(
            "yes",
...
def test_supersede_run_history_question(orchestration_service):
    params = {
        "goal": "Search HubSpot for high-intent leads and draft Slack follow-up",
        "steps": [],
    }
    task_state = {
        "clarified_params": params,
        "pending_task": {
            ...
            "status": "awaiting_step_confirm",
            "params": params,
            "current_step": {"label": "Search contacts"},
        },
    }
    assert (
        orchestration_service._should_supersede_pending_orchestration(
...
@pytest.mark.asyncio
async def test_completed_plan_unrelated_then_yes_does_not_retrigger(orchestration_service):
    """Regression: complete plan → unrelated Q → bare yes must not revive orch."""
    completed_state = {
        "clarified_params": {
            "goal": "Search HubSpot then Slack",
            "steps": [{"id": "step_1", "label": "Search contacts"}],
            "step_results": [{"success": True, "label": "Search contacts"}],
        },
        "pending_task": {
            "type": "connector_orchestration",
            "status": "completed",
        ...
        "current_plan": {
            ...
            "steps": [{"label": "Search contacts"}, {"label": "Draft Slack"}],
        },
        "pending_steps": [],
    }
    cleared = {
    ...
    # First turn: unrelated question while sticky completed orch + plan exist.
    orchestration_service._state.get_task_state = AsyncMock(
        side_effect=[completed_state, cleared, cleared]
    )
    orchestration_service._state.update_task_state = AsyncMock()
    orchestration_service._start_execution = AsyncMock()
    orchestration_service._execute_current_step = AsyncMock()

    unrelated = await orchestration_service.process_turn(
        org_id="org-1",
        user_id="user-1",
        conversation_id="conv-stale-plan",
        message="What workflows have been ran?",
    ...
    clear_patch = orchestration_service._state.update_task_state.await_args.args[2]
    assert clear_patch.get("pending_task") is None
    assert clear_patch.get("current_plan") is None
    orchestration_service._start_execution.assert_not_awaited()
    orchestration_service._execute_current_step.assert_not_awaited()

    # Second turn: coincidental bare "yes" after close-out — must not re-trigger.
    orchestration_service._state.get_task_state = AsyncMock(return_value=cleared)
    orchestration_service._state.update_task_state = AsyncMock()
    yes_turn = await orchestration_service.process_turn(
        org_id="org-1",
        user_id="user-1",
        conversation_id="conv-stale-plan",
        message="yes",
    ...
    assert yes_turn is None
    orchestration_service._start_execution.assert_not_awaited()
    orchestration_service._execute_current_step.assert_not_awaited()


@pytest.mark.asyncio
async def test_awaiting_step_unrelated_run_history_supersedes(orchestration_service):
    """Unrelated run-history ask must clear awaiting_step_confirm, not remind/approve."""
    params = {
        "goal": "Search HubSpot contacts then Slack",
        "steps": [{"id": "step_1", "label": "Search contacts"}],
        "current_step_index": 0,
        "total_steps": 2,
    }
    ...
    orchestration_service._state.get_task_state = AsyncMock(
        side_effect=[awaiting, cleared, cleared]
    )
    orchestration_service._state.update_task_state = AsyncMock()
    orchestration_service._execute_current_step = AsyncMock()
    ...
    result = await orchestration_service.process_turn(
        org_id="org-1",
        user_id="user-1",
        conversation_id="conv-supersede-runs",
        message="What workflows have been ran?",
        classification={"intent": "question"},
        task_state=awaiting,
        connected_integrations=["hubspot", "slack"],
