...
def test_pagerduty_actions_registered():
    ...
    assert "pagerduty.incidents.get" in actions
    assert "pagerduty.incidents.list" in actions
    assert "pagerduty.incidents.notes.list" in actions
    assert "pagerduty.incidents.resolve" in actions
    assert "pagerduty.incidents.reassign" in actions
    assert "pagerduty.services.list" in actions
    assert "pagerduty.oncalls.list" in actions
...
def test_pagerduty_incidents_get_success():
    settings = SimpleNamespace(disable_connectors=False, connector_secrets_encryption_key="k" * 32, encryption_key="")
    tool_ctx = ToolContext(
        settings=settings,
        client=MagicMock(),
    ...
    with patch("app.services.tool_service.get_connector_by_type", return_value=conn):
        with patch("app.services.tool_service.enforce_rate_limit"):
            with patch(
                ...
                with patch(
                    "app.services.tool_service.get_incident",
                    return_value={"id": "P1", "status": "triggered"},
                ):
                    with patch("app.services.tool_service.write_audit_event"):
                        with patch(
                            "app.services.agent_tool_permissions.list_agent_tool_permissions",
                            return_value=[
                                {
                                    "connector_type": "pagerduty",
                                    "scopes": ["pagerduty:incidents:read"],
                                    "expires_at": None,
                                }
                            ],
                        ):
    ...
    assert result.success is True
    assert result.data["incident"]["id"] == "P1"


def test_pagerduty_incidents_list_read_scope():
    settings = SimpleNamespace(disable_connectors=False, connector_secrets_encryption_key="k" * 32, encryption_key="")
    tool_ctx = ToolContext(
        settings=settings,
        client=MagicMock(),
    ...
    with patch("app.services.tool_service.get_connector_by_type", return_value=conn):
        with patch("app.services.tool_service.enforce_rate_limit"):
            with patch(
                ...
                with patch(
                    "app.services.tool_service.list_incidents",
                    return_value={"incidents": [{"id": "P2"}], "more": False},
                ):
                    with patch("app.services.tool_service.write_audit_event"):
                        with patch(
                            "app.services.agent_tool_permissions.list_agent_tool_permissions",
...
def test_pagerduty_resolve_requires_incident_id():
    settings = SimpleNamespace(disable_connectors=False, connector_secrets_encryption_key="k" * 32, encryption_key="")
    tool_ctx = ToolContext(
        settings=settings,
        client=MagicMock(),
            ...
            with patch(
                ...
                with patch("app.services.tool_service.write_audit_event"):
                    with patch(
                        ...
                        result = invoke_tool(tool_ctx, "pagerduty.incidents.resolve", {})
    assert result.success is False
    assert result.error_code == "validation_error"
