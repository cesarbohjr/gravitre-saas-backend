...
def test_workflow_optimizations_hides_acknowledged_insights(monkeypatch):
    """Apply/accepted feedback must remove Optimize cards on reload."""
    service = MesonService(model_router=object())  # type: ignore[arg-type]
    client = MagicMock()
    ...
    result = service.get_workflow_optimizations(
        ...
        environment_name="default",
        feedback_summary={
            "acknowledged_ids": {"publish-this-workflow", "meson-workflow-ready"},
            "dismissed_ids": set(),
        },
    )
    ids = {i.id for i in result.insights}
    assert "publish-this-workflow" not in ids
    assert "meson-workflow-ready" not in ids
