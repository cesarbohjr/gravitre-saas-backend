...
def test_former_lookalike_gaps_are_gated():
    ...
    for name in (
        ...
        assert requires is True, f"{name} must require approval (action={action})"
        blocked = block_react_write_execution(name, {}, registry)
...
def test_enumerate_all_catalog_mutating_registry_tools_are_gated():
    """Full sweep: every catalog-mutating tool with a registry key must gate. Zero gaps."""
    registry = get_tool_registry()
