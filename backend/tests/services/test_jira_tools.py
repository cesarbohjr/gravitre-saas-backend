...
def test_jira_actions_registered():
    ...
    assert "jira.issues.get" in actions
    assert "jira.issues.search" in actions
    assert "jira.issues.update" in actions
    assert "jira.issues.transitions.list" in actions
    assert "jira.projects.list" in actions
    assert "jira.users.search" in actions
...
def test_jira_issues_search_builds_jql_from_filters():
    settings = SimpleNamespace(disable_connectors=False, connector_secrets_encryption_key="k" * 32, encryption_key="")
    tool_ctx = ToolContext(
        settings=settings,
        client=MagicMock(),
    ...
    with patch("app.services.tool_service.get_connector_by_type", return_value=conn):
        with patch("app.services.tool_service.enforce_rate_limit"):
            with patch(
                ...
                with patch(
                    "app.services.tool_service.search_issues",
                    return_value={"issues": []},
                ) as searched:
                    with patch("app.services.tool_service.write_audit_event"):
                        with patch(
                            ...
                            result = invoke_tool(
                                tool_ctx,
                                "jira.issues.search",
                                {"project_key": "ENG", "status": "Open"},
                            )
    assert result.success is True
    searched.assert_called_once()
    assert 'project = "ENG"' in searched.call_args.args[2]
