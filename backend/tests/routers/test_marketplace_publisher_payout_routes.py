"""MKT-AUDIT-12.2: Creator revenue share payout routes."""
from __future__ import annotations

import pytest
from fastapi import HTTPException, status
from fastapi.testclient import TestClient

from app.auth.dependencies import get_current_user, get_environment_context, get_org_context, require_admin
from app.config import Settings, get_settings
from app.main import app

client = TestClient(app)

ORG_ID = "org-11111111-1111-1111-1111-111111111111"


def _settings() -> Settings:
    return Settings(
        app_env="dev",
        supabase_url="https://test.supabase.co",
        supabase_anon_key="anon-test",
        supabase_service_role_key="service-role-test",
        supabase_jwt_secret="jwt-secret-test",
        stripe_secret_key="sk_test",
    )


@pytest.fixture(autouse=True)
def _clear_overrides():
    yield
    app.dependency_overrides.clear()


def _authenticate_admin(org_id: str = ORG_ID) -> None:
    app.dependency_overrides[get_current_user] = lambda: {"user_id": "user-1", "email": "u@example.com"}
    app.dependency_overrides[get_org_context] = lambda: org_id
    app.dependency_overrides[get_settings] = lambda: _settings()
    app.dependency_overrides[get_environment_context] = lambda: "production"
    app.dependency_overrides[require_admin] = lambda: ({"user_id": "user-1"}, org_id)


def _deny_admin() -> tuple[dict, str]:
    raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin role required")


def test_publisher_payout_sync_requires_admin():
    _authenticate_admin()
    app.dependency_overrides[require_admin] = _deny_admin
    response = client.post("/api/marketplace/publisher/payouts/sync")
    assert response.status_code == 403


def test_publisher_payout_sync_route(monkeypatch):
    _authenticate_admin()
    monkeypatch.setattr(
        "app.routers.marketplace.sync_pending_payouts",
        lambda *_a, **_k: {"transferred": 1, "failed": 0, "pendingReviewed": 1},
    )
    monkeypatch.setattr(
        "app.routers.marketplace.get_publisher_payout_summary",
        lambda *_a, **_k: {
            "grossCents": 5000,
            "platformFeeCents": 1000,
            "partnerEarningsCents": 4000,
            "transferredCents": 4000,
            "pendingTransferCents": 0,
            "payoutCount": 1,
        },
    )
    monkeypatch.setattr("app.routers.marketplace.write_audit_event", lambda *_a, **_k: None)
    response = client.post("/api/marketplace/publisher/payouts/sync")
    assert response.status_code == 200
    body = response.json()
    assert body["sync"]["transferred"] == 1
    assert body["summary"]["partnerEarningsCents"] == 4000
