"""Salesforce inbound webhook receiver (STA-32)."""
from __future__ import annotations

import json
import logging
from typing import Annotated, Any

from fastapi import APIRouter, Depends, Header, HTTPException, Request, status
from supabase import create_client

from app.config import Settings, get_settings
from app.connectors.salesforce_webhooks import verify_salesforce_webhook_signature
from app.services.salesforce_trigger_service import (
    get_connector_webhook_secret,
    process_salesforce_event_batch,
)

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/api/webhooks/salesforce", tags=["salesforce-webhooks"])


@router.post("/inbound/{connector_id}")
async def salesforce_inbound_webhook(
    connector_id: str,
    request: Request,
    x_gravitre_signature: Annotated[str | None, Header(alias="X-Gravitre-Signature")] = None,
    settings: Settings = Depends(get_settings),
) -> dict[str, Any]:
    body = await request.body()
    client = create_client(settings.supabase_url, settings.supabase_service_role_key)
    row = (
        client.table("connectors")
        .select("id,type,config")
        .eq("id", connector_id)
        .limit(1)
        .execute()
    )
    if not row.data:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Connector not found")
    connector = dict(row.data[0])
    if str(connector.get("type") or "").lower() != "salesforce":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Not a Salesforce connector")

    secret = get_connector_webhook_secret(connector)
    if not secret:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Salesforce webhook secret not configured; reconnect OAuth",
        )
    if not verify_salesforce_webhook_signature(
        body=body,
        signature=x_gravitre_signature,
        secret=secret,
    ):
        logger.warning("salesforce_webhook_signature_invalid connector_id=%s", connector_id)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid signature")

    try:
        payload = json.loads(body.decode("utf-8")) if body else {}
    except json.JSONDecodeError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid JSON") from exc

    events: list[dict[str, Any]]
    if isinstance(payload, list):
        events = [dict(item) for item in payload if isinstance(item, dict)]
    elif isinstance(payload, dict):
        events = [payload]
    else:
        events = []

    results = await process_salesforce_event_batch(settings, connector_id, events)
    return {"received": len(events), "dispatched": len(results), "results": results}
