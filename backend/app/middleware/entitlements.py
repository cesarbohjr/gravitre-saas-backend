...
def resolve_entitlements(settings: Settings, org_id: str) -> dict[str, Any]:
    # Single source of truth: billing service plan + org billing status.
    client = get_supabase_client(settings)
    plan = get_plan_for_org(client, org_id)
    org_billing = get_org_billing(client, org_id) or {}
    subscription = _select_latest_subscription(client, org_id)

    tier = _normalize_tier(plan.get("code") or org_billing.get("plan_code"))
    status = str(org_billing.get("billing_status") or (subscription or {}).get("status") or "inactive")
    ...
    limits = {
        ...
        "lite_seats_included": max(
            int(tier_defaults.get("lite_seats_included") or 0),
            lite_seats,
        ),
    ...
    features = {
        # Legacy/UI features
        "custom_webhooks": _as_bool(plan_features.get("advanced_connectors")),
        "sso_saml": tier == "command",
        "api_full_access": tier in {"control", "command"},
        "priority_support_email": tier in {"control", "command"},
        "priority_support_dedicated": tier == "command",
        # Backend-enforced features
        "approvals": _as_bool(plan_features.get("approvals")),
        "audit_logs": _as_bool(plan_features.get("audit_logs")),
        "versioning": _as_bool(plan_features.get("versioning")),
        "advanced_connectors": _as_bool(plan_features.get("advanced_connectors")),
        "rbac": _as_bool(plan_features.get("rbac")),
    }
