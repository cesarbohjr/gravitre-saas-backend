...
def build_dynamic_chat_tool_specs() -> dict[str, AgentToolSpec]:
    ...
    for entry in build_connector_execution_matrix():
        ...
        description = (
            ...
            f"Connector: {entry.connector_id}. "
            f"Invoke action `{registry_key}`. "
            f"{'Read-only' if entry.kind == 'read' else 'Write — may require human approval'}."
        )
        specs[entry.tool_registry_key] = AgentToolSpec(
            name=entry.tool_registry_key,
            description=description,
            parameters=parameters_for_action(entry.action_key, kind=entry.kind, suffix=suffix),
