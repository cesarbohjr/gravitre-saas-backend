        ...
        *,
        ...
        if query:
            try:
                ...
                decision_recs = list(decision_payload.get("recommendations") or [])
        ...
        dept_brief = await self._signals.generate_department_brief(org_id, dept, client=client)
        daily = await self._user_intel.get_daily_briefing(
            self.settings,
            org_id=org_id,
            user_id=user_id,
        )
