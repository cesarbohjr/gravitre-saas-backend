...
-- 1) Verify no unexpected org_ids exist in demo data tables.
-- Allowed orgs:
--   - Primary demo org:   00000000-0000-0000-0000-000000000001
--   - Secondary demo org: 11111111-1111-4111-8111-111111111111
-- Any non_demo_count > 0 means stray org data exists in that table.
WITH constants AS (
  SELECT
    '00000000-0000-0000-0000-000000000001'::uuid AS primary_demo_org_id,
    '11111111-1111-4111-8111-111111111111'::uuid AS secondary_demo_org_id
)
SELECT 'users' AS entity, COUNT(*)::bigint AS non_demo_count
FROM public.users u, constants c
WHERE u.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
UNION ALL
SELECT 'agents', COUNT(*)::bigint
FROM public.agents a, constants c
WHERE a.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
UNION ALL
SELECT 'workflows', COUNT(*)::bigint
FROM public.workflows w, constants c
WHERE w.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
UNION ALL
SELECT 'runs', COUNT(*)::bigint
FROM public.runs r, constants c
WHERE r.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
UNION ALL
SELECT 'run_steps', COUNT(*)::bigint
FROM public.run_steps rs, constants c
WHERE rs.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
UNION ALL
SELECT 'approvals', COUNT(*)::bigint
FROM public.approvals a, constants c
WHERE a.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
...
SELECT 'connected_systems', COUNT(*)::bigint
FROM public.connected_systems cs, constants c
WHERE cs.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
UNION ALL
SELECT 'goals', CASE WHEN to_regclass('public.goals') IS NULL THEN 0 ELSE (
  SELECT COUNT(*)::bigint FROM public.goals g, constants c WHERE g.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
) END
UNION ALL
SELECT 'goal_plans', CASE WHEN to_regclass('public.goal_plans') IS NULL THEN 0 ELSE (
  SELECT COUNT(*)::bigint FROM public.goal_plans gp, constants c WHERE gp.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
) END
UNION ALL
SELECT 'council_sessions', CASE WHEN to_regclass('public.council_sessions') IS NULL THEN 0 ELSE (
  SELECT COUNT(*)::bigint FROM public.council_sessions cs, constants c WHERE cs.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
) END
UNION ALL
SELECT 'council_contributions', CASE WHEN to_regclass('public.council_contributions') IS NULL THEN 0 ELSE (
  SELECT COUNT(*)::bigint FROM public.council_contributions cc, constants c WHERE cc.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
) END
UNION ALL
SELECT 'workflow_health_snapshots', CASE WHEN to_regclass('public.workflow_health_snapshots') IS NULL THEN 0 ELSE (
  SELECT COUNT(*)::bigint FROM public.workflow_health_snapshots whs, constants c WHERE whs.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
) END
UNION ALL
SELECT 'optimization_recommendations', CASE WHEN to_regclass('public.optimization_recommendations') IS NULL THEN 0 ELSE (
  SELECT COUNT(*)::bigint FROM public.optimization_recommendations o, constants c WHERE o.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
) END
UNION ALL
SELECT 'workflow_ab_tests', CASE WHEN to_regclass('public.workflow_ab_tests') IS NULL THEN 0 ELSE (
  SELECT COUNT(*)::bigint FROM public.workflow_ab_tests w, constants c WHERE w.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
) END
UNION ALL
SELECT 'model_settings', COUNT(*)::bigint
FROM public.model_settings ms, constants c
WHERE ms.org_id NOT IN (c.primary_demo_org_id, c.secondary_demo_org_id)
ORDER BY entity;
